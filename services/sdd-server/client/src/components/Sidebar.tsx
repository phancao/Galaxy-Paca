...
import { OIDC_ENABLED, getUser, logout } from "../lib/oauth";
...
const ALL_NAV_KEYS = [
  ...
  { to: "/sdd", icon: ListChecks, key: "nav:sdd" },
  { to: "/cc-config", icon: Boxes, key: "nav:ccConfig" },
  { to: "/run", icon: Play, key: "nav:run" },
  { to: "/settings", icon: Settings, key: "nav:settings" },
] as const;

// The central coordination server reuses this UI but only the team-relevant
// surfaces make sense online; the per-machine monitor tabs (Dashboard, Kanban,
// Analytics, Workflows, Claude Config, Run) read local-only data, so hide them.
const CENTRAL_NAV = new Set([
  "/",
  "/kanban",
  "/sdd",
  "/sessions",
...
const NAV_KEYS = OIDC_ENABLED ? ALL_NAV_KEYS.filter((n) => CENTRAL_NAV.has(n.to)) : ALL_NAV_KEYS;
// On the central server the reused routes carry team meanings — relabel them.
...
export function Sidebar({ wsConnected, collapsed, onToggle }: SidebarProps) {
  // ADR-027: swap the Galaxy wordmark <img> for the logged-in tenant's logo.
  useEffect(() => {
    try {
      const t = localStorage.getItem('auth_token');
      const c = t ? JSON.parse(atob((t.split('.')[1] || '').replace(/-/g, '+').replace(/_/g, '/'))) : null;
      const tn = c && c.tenant;
      if (!tn) return;
      fetch(`/api/identity/public/auth/tenant/${encodeURIComponent(tn)}`, { cache: 'no-store' }).then((r) => (r.ok ? r.json() : null)).then((b) => {
        if (!b || !b.logo_url) return;
        const sw = () => { Array.from(document.getElementsByTagName('img')).forEach((el) => { const s = el.src || ''; if (s.indexOf('galaxy-technology') > -1) { el.removeAttribute('srcset'); el.src = b.logo_url; } }); };
        sw(); setTimeout(sw, 800); setTimeout(sw, 2000);
      }).catch(() => {});
    } catch { /* ignore */ }
  }, []);
  ...
  return (
    ...
    >
      {/* Brand — Galaxy Technology wordmark + product label. Logo Guide
          forbids effects / distortion, so we drop the prior Activity
          icon-on-tinted-tile glyph and use the official PNG straight at
          h-5 with native aspect. Path served from the gateway sub-path
          /sdd-server, hence the basePath-prefixed URL. */}
      <div className="px-3 py-4 border-b border-border flex-shrink-0">
        <div className={`flex items-center ${collapsed ? "justify-center" : "gap-3 px-2"}`}>
          <img
            src="/sdd-server/brand/galaxy-technology.png"
            alt="Galaxy Technology"
            className="h-5 w-auto shrink-0"
          />
          {!collapsed && (
            <>
              <div className="min-w-0 flex-1">
                <h1 className="text-sm font-semibold text-fg truncate">/ Coordination</h1>
                <p className="text-[11px] text-fg-subtle">{t("nav:brandSub")}</p>
              </div>
              {/* Theme toggle sits alongside the brand block when the
                  sidebar is expanded; collapsed mode hides it. */}
              <ThemeToggle />
            </>
          )}
        </div>
      ...
      <div className="flex-1 min-h-0 relative flex">
        <nav ref={navRef} className="flex-1 overflow-y-auto overflow-x-hidden px-2 py-3 space-y-1">
          {NAV_KEYS.map(({ to, icon: Icon, key }) => {
            const label = (OIDC_ENABLED && CENTRAL_LABEL[to]) || t(key);
            return (
              <NavLink
                key={to}
                to={to}
      ...
      >
        ...
        {OIDC_ENABLED && (
          <div className={collapsed ? "flex flex-col items-center gap-2 pt-0.5" : "space-y-1.5"}>
            {!collapsed && (
              <div className="flex items-center gap-2 rounded-lg border border-border bg-surface-2 px-2.5 py-2 text-xs text-gray-300">
                <UserCircle2 className="h-4 w-4 flex-shrink-0 text-indigo-300" />
                <span className="truncate font-medium">
                  {getUser()?.name || getUser()?.email || "Signed in"}
                </span>
              </div>
            )}
            <button
              ...
              className={
                collapsed
                  ? "flex h-8 w-8 items-center justify-center rounded-lg border border-transparent text-gray-400 hover:border-border hover:bg-surface-3 hover:text-rose-300"
                  : "group flex w-full items-center gap-2.5 rounded-lg border border-transparent px-2.5 py-2 text-xs text-gray-300 transition-colors hover:border-border hover:bg-surface-3 hover:text-rose-300"
              }
            >
              <span
                className={
                  collapsed
                    ? ""
                    : "flex h-6 w-6 items-center justify-center rounded-md bg-surface-3"
                }
              >
