...
import sdd_en from "./locales/en/sdd.json";
import sdd_zh from "./locales/zh/sdd.json";
import sdd_vi from "./locales/vi/sdd.json";
...
i18n
  ...
  .init({
    ...
    // Galaxy fork: VN-first. fallback → "vi" (catalogue is complete);
    // per-user picker still overrides for English readers.
    fallbackLng: "vi",
    ns: [
      "common",
      "nav",
    ...
    detection: {
      // Read the Galaxy-shared NEXT_LOCALE cookie first so flipping
      // language at the portal switcher propagates to SDD Server.
      // Local i18nextLng stays as second source for anyone who
      // already picked here.
