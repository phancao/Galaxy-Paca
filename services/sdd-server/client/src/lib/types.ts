...
export interface SddSessionDetail {
  session_id: string;
  phases: SddPhase[];
  activity: SddActivity[];
  gates: SddGate[];
}

export interface SddUpdatePayload {
  session_id?: string;
  phase?: string | null;
  phase_index?: number | null;
  level?: number | null;
  lifecycle?: string | null;
  shared_core_touch?: boolean;
  spec?: { docId: string | null; version: string | null } | null;
  gate?: SddGate;
  specSync?: { upserted: number };
  created_at?: string;
}
