...
import { OIDC_ENABLED, getToken, clearToken, beginLogin } from "./oauth";

// Prefix every API call with the app's base path so the client works both
// standalone (base "/") and mounted behind a gateway sub-path (e.g.
// "/sdd-monitor/"). import.meta.env.BASE_URL is Vite's configured base.
const BASE = `${import.meta.env.BASE_URL.replace(/\/$/, "")}/api`;

// One-shot 401-recovery guard. Without this, parallel /api/* calls that
// all 401 at the same time each trigger their own beginLogin(), racing to
// navigate. The first wins, the others log "login required" errors that
// React surfaces mid-redirect. Mirrors the Galaxy Vortex canonical pattern.
let reauthing = false;
...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  // Attach the Vortex OIDC access token when present (central server). The
  // per-machine monitor has no token and sends none.
  const token = OIDC_ENABLED ? getToken() : null;
  ...
  if (res.status === 401 && OIDC_ENABLED) {
    // Session expired / not logged in → clear + bounce to Vortex login.
    // The guard ensures concurrent 401s collapse into ONE navigation.
    reauthenticate();
...
export const api = {
  ...
  team: {
    ...
    analytics: () => request<TeamAnalytics>("/team/analytics"),
    fleet: () => request<{ machines: FleetMachine[] }>("/team/fleet"),
    coordination: () => request<TeamCoordination>("/team/coordination"),
  },

  tasks: {
    list: () => request<TasksResult>("/tasks"),
    assignees: () => request<{ assignees: TaskAssignee[] }>("/tasks/assignees"),
  ...
  sdd: {
    overview: () => request<SddOverview>("/sdd"),
    session: (id: string) => request<SddSessionDetail>(`/sdd/session/${encodeURIComponent(id)}`),
    specVersions: () => request<SddSpecVersionsResult>("/sdd/spec-versions"),
    flags: () => request<SddFlagsResult>("/sdd/flags"),
    git: (sessionId: string) =>
      request<{ session_id: string; cwd: string | null; git: unknown }>(
        `/sdd/git?session_id=${encodeURIComponent(sessionId)}`
      ),
    setGate: (body: {
      session_id: string;
      phase: string;
      ...
      request<{ ok: boolean; gate: SddGate }>("/sdd/gates", {
        method: "POST",
        body: JSON.stringify(body),
      }),
    specSync: () =>
      request<{ configured: boolean; fetched: number; upserted: number; error?: string }>(
        "/sdd/spec-sync",
        { method: "POST", body: JSON.stringify({}) }
      ),
    specSyncStatus: () =>
      request<{ configured: boolean; hasToken: boolean }>("/sdd/spec-sync/status"),
  },
